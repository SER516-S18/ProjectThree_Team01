...
import client.gui.panels.ColorChooserPanel;
  ...
  public void mouseClicked(MouseEvent arg0) {
    ...
    if (actionClass instanceof ColorChooserPanel) {
      if (switcher == "Color Panel") {
        ...
        XYPlot plot = ((ColorChooserPanel) actionClass).box.parent.performanceGraph.getGraph()
            .getXYPlot();
        plot.getRenderer().setSeriesPaint(((ColorChooserPanel) actionClass).box.boxNumber,
            ((JPanel) arg0.getSource()).getBackground());
        ((ColorChooserPanel) actionClass).setVisible(false);
        ((ColorChooserPanel) actionClass).dispose();

      } else if (switcher == "Close") {
        ((ColorChooserPanel) actionClass).box.setBoxColor(((ColorChooserPanel) actionClass).colorArray
            .get(((ColorChooserPanel) actionClass).colorArray.size() - 1));
        ((ColorChooserPanel) actionClass).setVisible(false);
        ((ColorChooserPanel) actionClass).dispose();
      }
    }
