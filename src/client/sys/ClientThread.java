...
/**
 * Thread that connects to the server, waits till the client closes, closes the session
...
public class ClientThread implements Runnable {
  ...
  /**
   * Connect to server using the uri and port entered by the user.
   */
  @Override
  public void run() {
    ClientManager client = ClientManager.createClient();
    try {
      Session session = client.connectToServer(ClientWebSocket.class,
          new URI(Constants.PROTOCOL + uri + ':' + port + Constants.LINK + Constants.ENDPOINT));
